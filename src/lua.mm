...
void lua::bindMethod(lua_State* L, Class cls, Method method) {
	...
	if (strncmp(className, "Test", 4) == 0 || strncmp(className, "test", 4) == 0) return;
...
void lua::bindObjc(lua_State* L) {
	...
	for (unsigned int i = 0; i < numClasses; i++) {
		...
		if ((strncmp(className, "__", 2) == 0 || (className[0] == '_' && className[1] != '_')) || strcmp(className, "Object") == 0 || strncmp(className, "CK", 2) == 0) continue;
